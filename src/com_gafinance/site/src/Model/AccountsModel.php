...
/**
 * Methods supporting a list of Gafinance records.
 *
 * @since  1.6
 */
class AccountsModel extends ListModel
{
	...
	public function __construct($config = array())
	{
		if (empty($config['filter_fields']))
		{
			$config['filter_fields'] = array(
				...
				'accnt_name', 'a.accnt_name',
				'accnt_bsb', 'a.accnt_bsb',
				'accnt_number', 'a.accnt_number',
				'comment', 'a.comment',
			);
		}
	...
	protected function getListQuery()
	{
        ...
        $query->from('#__gafinance_accounts AS a');
        ...
        if (!empty($search)) {
            if (stripos($search, 'id:') === 0) {
				...
				$query->where('( a.accnt_name LIKE ' . $search . ' )');
            }
        }

        // Add the list ordering clause.
        $orderCol  = $this->state->get('list.ordering', "a.accnt_name");
