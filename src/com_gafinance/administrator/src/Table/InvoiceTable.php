...
class InvoiceTable extends Table implements VersionableTableInterface
{
	...
	{
		$this->typeAlias = 'com_gafinance.invoice';
		parent::__construct('#__gafinance_invoices', 'id', $db);
	...
	public function bind($array, $ignore = '')
	{
		...
		if (!isset($array['inv_date']) || empty($array['inv_date'])) {
			$array['inv_date'] = null;
		}
		...
		if (!$user->authorise('core.admin', 'com_gafinance.invoice.' . $array['id'])) {
			$actions         = Access::getActionsFromFile(
				JPATH_ADMINISTRATOR . '/components/com_gafinance/access.xml',
				"/access/section[@name='invoice']/"
			);
			$default_actions = Access::getAssetRules('com_gafinance.invoice.' . $array['id'])->getData();
			$array_jaccess   = array();
	...
	protected function _getAssetName()
	{
		...
		return 'com_gafinance.invoice.' . (int) $this->$k;
	}
