 ...
 * @since  1.5
 */
class AuditTable extends Table implements VersionableTableInterface
{
	/**
	 * Constructor
	...
	{
		$this->typeAlias = 'com_gafinance.audit';
		parent::__construct('#__gafinance_audit_trail', 'id', $db);
	...
	public function bind($array, $ignore = '')
	{
		...
		if (!$user->authorise('core.admin', 'com_gafinance.audit.' . $array['id'])) {
			$actions         = Access::getActionsFromFile(
				JPATH_ADMINISTRATOR . '/components/com_gafinance/access.xml',
				"/access/section[@name='audit']/"
			);
			$default_actions = Access::getAssetRules('com_gafinance.audit.' . $array['id'])->getData();
			$array_jaccess   = array();
	...
	protected function _getAssetName()
	{
		...
		return 'com_gafinance.audit.' . (int) $this->$k;
	}
