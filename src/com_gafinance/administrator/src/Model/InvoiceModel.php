...
class InvoiceModel extends AdminModel
{
	 ...
	 * @since    1.6
	 */
	protected $text_prefix = 'COM_GAFINANCE';

	/**
	 * @var   	string  	Alias to manage history control
	 * @since   3.2
	 */
	public $typeAlias = 'com_gafinance.invoice';
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_gafinance.edit.invoice.data', array());
	...
	public function duplicate(&$pks)
	{
		...
		// Access checks.
		if (!$user->authorise('core.create', 'com_gafinance')) {
	...
	protected function prepareTable($table)
	{
		if (empty($table->id)) {
			...
			if (@$table->ordering === '') {
				...
				$db->setQuery('SELECT MAX(ordering) FROM #__gafinance_invoices');
				$max             = $db->loadResult();
	...
	{
		$user = GafinanceHelper::getSpecificUser();
        $dateFldName = 'paid_date';

		// Access checks.
		if (!$user->authorise('core.manage', 'com_gafinance')) {
        ...
        {
            $db = Factory::getContainer()->get('DatabaseDriver');
            $db->setQuery('UPDATE #__gafinance_invoices SET '.$dateFldName.' = NULL WHERE id = '.(int) $pk);
            try {
        		$db->execute();
