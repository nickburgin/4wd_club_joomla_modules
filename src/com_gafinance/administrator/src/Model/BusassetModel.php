 ...
 * @since  1.6
 */
class BusassetModel extends AdminModel
{
	/**
	 ...
	 * @since   3.2
	 */
	public $typeAlias = 'com_gafinance.busasset';
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm( 
			'com_gafinance.busasset', 'busasset',
			array('control' => 'jform',
			'load_data' => $loadData ) 
			);
	...
	protected function prepareTable($table)
	{
		if (empty($table->id)) {
			...
			if (@$table->ordering === '') {
				...
				$db->setQuery('SELECT MAX(ordering) FROM #__gafinance_busassets');
				$max             = $db->loadResult();
