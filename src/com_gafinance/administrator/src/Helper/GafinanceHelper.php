...
class GafinanceHelper
{
	 ...
	 * @return  string	account number/s, comma separated if multiples
	 */
	public static function getAllTransactions($startdate, $enddate, $rpt_accnt)
	{
		if (!$rpt_accnt) {
            return 0;
		}
        ...
        $query = $db->getQuery(true);
        $query->clear();
        $query->select( ' a.accnt_id, a.tran_type, c.title, a.tran_desc, SUM(a.tran_amount) as tranAmt ' );
        $query->select( ' a.state, b.accnt_name as accnt_id_name, u.name as user_id_name ' );
        $query->from('#__gafinance_transactions AS a ');
        $query->join('LEFT', '#__categories AS c ON c.id = a.cat_id ');
        $query->join('LEFT', '#__gafinance_accounts AS b ON b.id = a.accnt_id AND b.state = 1');
        ...
        $query->where(' a.accnt_id IN ('.$rpt_accnt.')');
	...
	/**
	 * Get the key details of the last reconciliation for an account
	 * @params  int Account ID reference
	 * @return  object
	 */
	public static function getLastReconciliation($enddate, $accnt)
	{
		// Get the last balance reset figure and date.
        $db = Factory::getContainer()->get('DatabaseDriver');
        $query = $db->getQuery(true);
        $query->clear();
        $query->select( " a.id, a.tran_amount, a.tran_date " );
        $query->from(' #__gafinance_transactions AS a ');
        $query->where(' a.state = 1 ');
        $query->where(' a.accnt_id = '.(int) $accnt);
        $query->where(' a.tran_type = "Z" ');
        $query->where(' a.tran_date <= '.$db->Quote($enddate) );
        $query->where(' a.tran_date >= (select max(tran_date) from #__gafinance_transactions where tran_type = "Z" and state = 1 and tran_date <= '.$db->Quote($enddate).' and accnt_id = '.(int) $accnt.' ) ' );
	 ...
	 */
	public static function getOpeningBalance($startdate, $enddate, $rpt_accnt = 1)
	{
        $params = ComponentHelper::getParams('com_gafinance');
        ...
        $combine_rpt = $params->get('combine_rpt', 0);
 		...
 		// Get the last balance reset figure and date.
        $reconbal = self::getLastReconciliation($enddate, $rpt_accnt);

		// Get total value of transactions from reconciliation date to start date.
        $db = Factory::getContainer()->get('DatabaseDriver');
        $query = $db->getQuery(true);
        $query->clear();
	...
	/**
	 * Get the closing balance for a given date range
	 */
	public static function getClosingBalance($startdate, $enddate, $rpt_accnt)
	{
