 ...
 * @since  1.5
 */
class AttendeeTable extends Table implements VersionableTableInterface
{
	/**
	 * Constructor
	...
	{
		$this->typeAlias = 'com_gacalevents.attendee';
		parent::__construct('#__gacalevents_attendees', 'id', $db);
	...
	{
		return 'com_gacalevents.attendee';
	}
	...
	public function bind($array, $ignore = '')
	{
		...
		if ($array['id'] > 0 & $array['pub_name'] != GacaleventsHelper::getSpecificUser($array['attendee'])->name) {
			$array['pub_name'] = GacaleventsHelper::getSpecificUser($array['attendee'])->name;
		}
		...
		if (!$user->authorise('core.admin', 'com_gacalevents.attendee.' . $array['id'])) {
			$actions         = Access::getActionsFromFile(
				JPATH_ADMINISTRATOR . '/components/com_gacalevents/access.xml',
				"/access/section[@name='attendee']/"
			);
			$default_actions = Access::getAssetRules('com_gacalevents.attendee.' . $array['id'])->getData();
			$array_Access   = array();
	...
	protected function _getAssetName()
	{
		...
		return 'com_gacalevents.attendee.' . (int) $this->$k;
	}
