...
class com_gacaleventsInstallerScript extends InstallerScript
{
	...
	public function preflight($type, $parent)
	{
		...
		$pathSite = Path::clean( JPATH_SITE . '/components/com_'.$this->compName.'/tmpl/attendeeform/' );
		$this->deleteFiles($pathSite, 'modalAttend', '.php');
	...
	public function postflight($type, $parent)
	{
		...
		if (STRTOUPPER($type) == 'UPDATE') {
			...
			$pathSQL = Path::clean( JPATH_ADMINISTRATOR . '/components/com_'.$this->compName.'/sql/updates/mysql/' );
			$this->deleteFiles($pathSQL, '1.3.0', '.sql');
			$this->deleteFiles($pathSQL, '3.0.1', '.sql');
