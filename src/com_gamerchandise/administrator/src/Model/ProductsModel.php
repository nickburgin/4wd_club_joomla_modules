...
/**
	...
	*/
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		parent::populateState('a.prod_name', 'ASC');
	...
	protected function getListQuery()
	{
		...
		// Join over the user field 'category'
		$query->select('cat.title AS cat_id');
		...
		if (!empty($search)) {
			if (stripos($search, 'id:') === 0) {
				...
				$query->where('( a.prod_name LIKE ' . $search . ' )');
			}
		}
		...
		if ($gender) {
			$query->where('a.gender = ' . (int) $gender);
		}
