 ...
 * @since  1.6
 */
class SaleModel extends AdminModel
{
	/**
	 * @var      string    The prefix to use with controller messages.
	 * @since    1.6
	 */
	protected $text_prefix = 'COM_GAMERCHANDISE';

	/**
	 * @var   	string  	Alias to manage history control
	 * @since   3.2
	 */
	public $typeAlias = 'com_gamerchandise.sale';
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm( 'com_gamerchandise.sale', 'sale', array('control' => 'jform', 'load_data' => $loadData ) );
	...
	protected function prepareTable($table)
	{
		if (empty($table->id)) {
			...
			if (@$table->ordering === '') {
				...
				$db->setQuery('SELECT MAX(ordering) FROM #__gamerchandise_sales');
				$max             = $db->loadResult();
