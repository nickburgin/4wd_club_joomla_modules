 ...
 * @since  1.6
 */
class SaleformModel extends FormModel
{
	private $item = null;
	...
	protected function populateState()
	{
		...
		if (Factory::getApplication()->input->get('layout') == 'edit') {
			...
			$prod_id = Factory::getApplication()->getUserState('com_gamerchandise.edit.sale.prod_id');
			//if ($id && !$prod_id)
			//$prod_id = Factory::getApplication()->input->get('prod_id', 0);
		} else {
		...
		$this->setState('sale.prod_id', $prod_id);
	...
	public function getItem($id = null)
	{
		...
		$public_sales = $params->get('public_sales', 0);
		$ignore_cat = $params->get('ignore_cat', 0);
		if ($this->item === null) {
			$this->item = false;

			if (empty($id)) {
				$id = $this->getState('sale.id');
				$prod_id = $this->getState('sale.prod_id');
			}
			...
			if ($prod_id) {
				$this->item->prod_id = $prod_id;
				$this->item->product = GamerchandiseHelper::getProductDetails($this->item->prod_id);
				$this->item->price = $this->item->product->price;
			}
		}
	...
	{
		// Get the form.
        $form = $this->loadForm('com_gamerchandise.sale', 'saleform', array(
                    'control'   => 'jform',
                    'load_data' => $loadData ) );
	...
	{
		$data = Factory::getApplication()->getUserState('com_gamerchandise.edit.sale.data', array());
	...
	public function save($data)
	{
		...
		$data['cat_size_id'] = (isset($data['cat_size_id'])) ? $data['cat_size_id'] : $data['catl_size_id'];
		...
		$data['ord_amt'] = $data['order_qty'] * $data['price'];
