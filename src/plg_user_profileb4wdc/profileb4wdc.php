 ...
 */
class PlgUserProfileb4wdc extends CMSPlugin
{
	/**
	 * Application object.
	 * @var    JApplicationCms
	...
	public function onContentPrepareForm(Form $form, $data)
	{
		...
		$fields = array(
			...
			'trg_profp', 'trg_lead', 'trg_instp', 'trg_csawp', 'trg_faidp', 'trg_trckp', 'trg_foodp',
	...
	function saveAuditTrail($data, $comment = '')
	{
		...
		try {
			$result = $this->db->insertObject('#__gausers_profile_audit', $audit);
			...
			if ($log_mbrsec && $this->app->isClient('site')) {
                ...
                if (is_array($mbrsecs)) {
					foreach ($mbrsecs AS $recip) {
                        $ru = GausersHelper::getSpecificUser($recip);
                        $recipients[] = $ru->email;
                    }
				} else {
                    $ru = GausersHelper::getSpecificUser($mbrsecs);
					$recipients[] = $ru->email;
				}
