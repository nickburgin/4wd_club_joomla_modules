...
if ($saveOrder)
{
	$saveOrderingUrl = 'index.php?option=com_gabroadcast&task=usernews.saveOrderAjax&tmpl=component';
	HTMLHelper::_('sortablelist.sortable', 'usernewList', 'adminForm', strtolower($listDirn), $saveOrderingUrl);
}
$sortFields = $this->getSortFields();

?>

<form action="<?php echo Route::_('index.php?option=com_gabroadcast&view=usernews'); ?>" method="post"
		...
		<div class="col-md-12">
			...
			<table class="table table-striped" id="usernewList">
				...
				<tr>
					...
					<th class='left'>
						<?php echo HTMLHelper::_('searchtools.sort',  'COM_GABROADCAST_USERNEWS_NEWS_SUBJECT', 'a.`news_subject`', $listDirn, $listOrder); ?>
					</th>
					<th class='left'>
						<?php echo HTMLHelper::_('searchtools.sort',  'COM_GABROADCAST_USERNEWS_CAT_ID', 'a.`cat_id`', $listDirn, $listOrder); ?>
					</th>
					<th class='left'>
				...
				<?php foreach ($this->items as $i => $item) :
					...
					<tr class="row<?php echo $i % 2; ?>">
						...
						<?php if (isset($this->items[0]->state)): ?>
							<td class="center">
								<?php echo HTMLHelper::_('jgrid.published', $item->state, $i, 'usernews.', $canChange, 'cb'); ?>
							</td>
						<?php endif; ?>
						...
						<?php endif; ?>
						<?php if ($canEdit) : ?>
							<a href="<?php echo Route::_('index.php?option=com_gabroadcast&task=usernew.edit&id='.(int) $item->id); ?>">
							<?php echo $this->escape($item->news_subject); ?></a>
						<?php else : ?>
							<?php echo $this->escape($item->news_subject); ?>
						<?php endif; ?>
		
						</td>
						<td><?php $pending_bcast = str_replace(",","\r\n", !empty($item->pending_bcast) ? $item->pending_bcast : ''); ?>
							<span title='<?php echo $pending_bcast; ?>'><?php echo $item->cat_id_name; ?></span>
						</td>
						<td>
