...
namespace GlennArkell\Component\Gabroadcast\Administrator\View\Usernews;
...
class HtmlView extends BaseHtmlView
{
	...
	public function display($tpl = null)
	{
        ...
        HTMLHelper::stylesheet(Uri::base().'media/com_gabroadcast/css/gabroadcast.css');
        HTMLHelper::stylesheet(Uri::base().'media/com_gabroadcast/css/list.css');
	...
	protected function addToolbar()
	{
		...
		$canDo = ContentHelper::getActions('com_gabroadcast','component',0);

		$customIcon = '';

		if (file_exists(JPATH_SITE . '/media/com_gabroadcast/images/l_'.$progNameL.'s.png')) {
			$customIcon = $progNameL.'s';
		}

		ToolbarHelper::title(Text::_('COM_GABROADCAST_TITLE_'.STRTOUPPER($progNameL).'S'), $customIcon);

		$toolbar = Toolbar::getInstance('toolbar');
        $toolbar->link('JTOOLBAR_DASHBOARD', 'index.php?option=com_cpanel&view=cpanel&dashboard=gabroadcast');

		// Check if the form exists before showing the add/edit buttons
		$formPath = JPATH_ADMINISTRATOR . '/components/com_gabroadcast/src/View/'.$progNameC.'s';
        ...
        $compVersion = GabroadcastHelper::getComponentVersion();
		...
		if ($canDo->get('core.admin')) {
			$toolbar->preferences('com_gabroadcast');
		}
	...
	protected function getSortFields()
	{
		return array(
			...
			'a.`created_date`' => Text::_('COM_GABROADCAST_CREATED_DATE'),
			'a.`modified_date`' => Text::_('COM_GABROADCAST_MODIFIED_DATE'),
		);
	}
