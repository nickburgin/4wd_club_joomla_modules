...
namespace GlennArkell\Component\Gatripsys\Administrator\View\Invoices;
...
class HtmlView extends BaseHtmlView
{
	...
	public function display($tpl = null)
	{
        ...
        HTMLHelper::stylesheet(Uri::base().'media/com_gatripsys/css/gatripsys.css');
        HTMLHelper::stylesheet(Uri::base().'media/com_gatripsys/css/list.css');
	...
	protected function addToolbar()
	{
		...
		$canDo = ContentHelper::getActions('com_gatripsys','component',0);
        $customIcon = '';
		if (file_exists(JPATH_SITE . '/media/com_gatripsys/images/l_'.$progNameL.'s.png')) {
			$customIcon = $progNameL.'s';
		}

		ToolbarHelper::title(Text::_('COM_GATRIPSYS_TITLE_'.STRTOUPPER($progNameL).'S'), $customIcon);

		$toolbar = Toolbar::getInstance('toolbar');
        $toolbar->link('JTOOLBAR_DASHBOARD', 'index.php?option=com_cpanel&view=cpanel&dashboard=gatripsys');

		// Check if the form exists before showing the add/edit buttons
		$formPath = JPATH_ADMINISTRATOR . '/components/com_gatripsys/src/View/'.$progNameC.'s';
		...
		if ($canDo->get('core.admin')) {
			$toolbar->preferences('com_gatripsys');
		}
