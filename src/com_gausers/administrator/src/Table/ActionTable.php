 ...
 * action Table class
 */
class ActionTable extends Table implements VersionableTableInterface
{
    /**
     * Constructor
     *
	...
	{
		$this->typeAlias = 'com_gausers.action';
		parent::__construct('#__gausers_actions', 'id', $db);
    ...
    public function bind($array, $ignore = '') 
	{
		...
		if (!$user->authorise('core.admin', 'com_gausers.action.' . $array['id'])) {
			$actions         = Access::getActionsFromFile(
				JPATH_ADMINISTRATOR . '/components/com_gausers/access.xml',
				"/access/section[@name='action']/"
			);
			$default_actions = Access::getAssetRules('com_gausers.action.' . $array['id'])->getData();
			$array_jaccess   = array();
    ...
    */
    protected function _getAssetName() {
        $k = $this->_tbl_key;
        return 'com_gausers.action.' . (int) $this->$k;
    }
