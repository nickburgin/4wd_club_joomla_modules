 ...
 * @since  1.5
 */
class MshiptypeTable extends Table implements VersionableTableInterface
{
	/**
	 * Constructor
	...
	{
		$this->typeAlias = 'com_gausers.mshiptype';
		parent::__construct('#__gausers_mshiptypes', 'id', $db);
	...
	public function bind($array, $ignore = '')
	{
		...
		if (!$user->authorise('core.admin', 'com_gausers.mshiptype.' . $array['id'])) {
			$actions         = Access::getActionsFromFile(
				JPATH_ADMINISTRATOR . '/components/com_gausers/access.xml',
				"/access/section[@name='mshiptype']/"
			);
			$default_actions = Access::getAssetRules('com_gausers.mshiptype.' . $array['id'])->getData();
			$array_Access   = array();
	...
	protected function _getAssetName()
	{
		...
		return 'com_gausers.mshiptype.' . (int) $this->$k;
	}
