...
namespace GlennArkell\Component\Gausers\Administrator\View\Actions;
...
class HtmlView extends BaseHtmlView
{
	...
	protected function addToolbar()
	{
		...
		if ($canDo->get('core.edit.state')  || count($this->transitions)) {
            ...
            // Add a batch button
            if (
                $user->authorise('core.create', 'com_'.$compName)
                && $user->authorise('core.edit', 'com_'.$compName)
            ) {
                $childBar->popupButton('batch', 'JTOOLBAR_BATCH')
                    ->popupType('inline')
                    ->textHeader(Text::_('COM_GAUSERS_BATCH_OPTIONS'))
        ...
        $compVersion = GausersHelper::getComponentVersion();
	...
	{
		return array(
			'member.name' => Text::_('COM_GAUSERS_FORM_LBL_ACTION_USER_ID'),
